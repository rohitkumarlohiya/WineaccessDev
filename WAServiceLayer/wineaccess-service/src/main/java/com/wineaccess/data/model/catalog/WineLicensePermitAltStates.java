...
@NamedQueries({
    ...
    @NamedQuery(name = "getPermitIdByWineId", query = "select wineryPermit.id,id from WineLicensePermitAltStates wia where wineId.id= :wineId"),
    @NamedQuery(name = "getDTCPermitNumberByWineId", query = "select dtcPermitNumber from WineLicensePermitAltStates wia where wineId.id= :wineId")
